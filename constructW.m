function W = constructW(fea,options) % input / options 
%	Usage:
...
switch lower(options.NeighborMode) % lower_case(knn);
...
if strcmpi(options.NeighborMode,'Supervised') % NeighbourMode === 'Supervised' ????
...
if strcmpi(options.NeighborMode,'KNN') && (options.k > 0) % NeighborMode == KNN ????
    if ~(bCosine && options.bNormalized) % if not (bCosine && bNormlize) 00 , 01 ,10 
    G = zeros(nSmp*(options.k+1),3);
        for i = 1:ceil(nSmp/BlockSize) % for 1 to 1
            if i == ceil(nSmp/BlockSize)  % i == 1 == 1
        ...
        W = sparse( G(:,1) , G(:,2) ,G(:,3),nSmp,nSmp); % 50 x 50 
    ...
    if bBinary
        W(logical(W)) = 1; % 0 => 0 , 1 and other non zero => 1
    end
